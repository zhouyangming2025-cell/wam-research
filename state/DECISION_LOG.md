...
1. `codex/canonical12-mechanism-reconstruction` contains a useful but explicitly provisional reconstruction: scoped artifacts, deployment/training lifecycle, candidate identity, paper/release boundaries, and collision tests. Its route compression remains `RECONSTRUCTED-BUT-NOT-NORMATIVE`; human acceptance and an unseen-paper test were pending. It is **not** restored as a route-map layer. The reusable guardrails were merged in place into `audits/research_synthesis/WAM_PLANNING_INTERFACE_FRAMEWORK.md`. The branch also exposed six provisional family hypotheses—direct planning with compiled world knowledge; retained online world/generator-state conditioning; online candidate-consequence evaluation; candidate selection with a compiled world criterion; reciprocal world–planner refinement; and semantic decision→action—with world/joint generation kept as terminal capabilities. They remain a held audit hypothesis, not an active main classification.
