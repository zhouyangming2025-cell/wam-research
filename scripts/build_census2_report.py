...
def main() -> int:
    ...
    a("3. **Cards**: these records deliberately have no paper cards (cards are a deep-read artifact),")
    a("   so their manifest rows carry a `NO_CARD` note. Batch-0A records P0003–P0007, P0008, P0009 and")
    a("   P0011 are in the same state: their manifest rows previously pointed at card files that do")
    a("   not exist on disk, and that was corrected in this batch.")
