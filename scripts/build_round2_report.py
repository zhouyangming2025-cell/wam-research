...
def main() -> int:
    ...
    a("layer, provenance/QC facts and card skeletons.")
    a("")
    ...
    a("occupancy judgement and no research verdict is recorded. Every scientific section of every")
    a("card is `PENDING SCIENTIFIC REVIEW` (deep-read / GPT-5.6 Sol work). No method design and no")
    ...
    a("so the batch ledgers and the manifest carry the label 2026-09-13 while this report and the")
    a("cards are stamped with the completion date.")
    a("")
    a("---")
    a("")
    a("## 1. ID allocation")
    ...
    a("`DOCUMENT_VERIFIED`. P0020 has no document, so it stays `INDEX_BACKED` and its card says so.")
    a("")
    a("## 5. Raw-MD QC")
    a("")
    ...
    a("recorded in the card but leaves the field `UNKNOWN` rather than `YES`.")
    a("")
    ...
    for pid in ids:
        ...
        a(f"- `papers/cards/{pid}_{m['short']}.md`")
    for pid in ids:
        if raw.get(pid):
    ...
    a("- `scripts/build_round2_cards.py`")
    ...
    a("3. **Card skeletons are infrastructure only.** Filling sections 1-12 and 14 is the deep-read")
    a("   task; the reading queue order was not changed by this ingest.")
    a("")
    return _write(L)
